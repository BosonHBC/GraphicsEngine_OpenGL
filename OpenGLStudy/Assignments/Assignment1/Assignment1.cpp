...
void Assignment1::UpdateBasedOnTime(float second_since_lastFrame)
{
	...
	s_clearColor.r = s_clearColor.r + (s_channelDir[0] ? 1.f : -1.f) * 1.f *second_since_lastFrame;
	// change g channel
	if (s_clearColor.g > 0.99f || s_clearColor.g < 0.01f) {
		s_channelDir[1] = !s_channelDir[1];
	}
	s_clearColor.g = s_clearColor.g + (s_channelDir[0] ? 1.f : -1.f) * 2.f *second_since_lastFrame;
	// change b channel
	if (s_clearColor.b > 0.99f || s_clearColor.b < 0.01f) {
		s_channelDir[2] = !s_channelDir[2];
	}
	s_clearColor.b = s_clearColor.b + (s_channelDir[0] ? 1.f : -1.f) * 3.f *second_since_lastFrame;
