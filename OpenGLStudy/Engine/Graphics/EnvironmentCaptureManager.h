...
namespace Graphics
{
	...
	namespace EnvironmentCaptureManager
	{
		struct sCaptureProbes
		{
			...
			sCaptureProbes() {}
			~sCaptureProbes() {}
			sCaptureProbes(const cSphere& i_sphere, const cSphere& i_innerSphere, float i_influence, GLuint i_resolution) : BV(i_sphere), InnerBV(i_innerSphere), Influence(i_influence), Resolution(i_resolution) { }
			sCaptureProbes(const sCaptureProbes& i_other) = default;
			sCaptureProbes& operator = (const sCaptureProbes& i_other) = default;
